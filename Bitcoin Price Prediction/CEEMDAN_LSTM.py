...
def declare_path(path=PATH,figure_path=FIGURE_PATH,log_path=LOG_PATH,dataset_name=DATASET_NAME,series=SERIES):
    ...
    plt.xlabel('Time')
    plt.legend()
    plt.savefig(FIGURE_PATH+'Original Dataset Figure.png', bbox_inches='tight')
    plt.show()
...
def emd_decom(series=None,trials=10,re_decom=False,re_imf=0,draw=True): 
    ...
    if draw:
        ...
        fig = plt.figure(figsize=(25,4*imfs_num))
        ...
        plt.tight_layout()
        if file_name == '':
            if (re_decom==False): plt.savefig(FIGURE_PATH+file_name+str.upper(MODE)+' Result.png', bbox_inches='tight')
            else: plt.savefig(FIGURE_PATH+'IMF'+str(re_imf)+' '+str.upper(MODE)+' Re-decomposition Result.png', bbox_inches='tight')
        plt.show()
...
def sample_entropy(imfs_df=None): # imfs_df is pd.DataFrame
    ...
    plt.legend()
    if file_name == '': fig.savefig(FIGURE_PATH+'Sample Entropy of %s IMFs.png'%(file_name), bbox_inches='tight')
    plt.show()
...
def integrate(df=None,inte_form=[[0,1],[2,3,4],[5,6,7]]):
    ...
    fig = plt.figure(figsize=(25,4*num)) 
    ...
    fig.align_labels()
    plt.tight_layout()
    plt.savefig(FIGURE_PATH+file_name+str.upper(MODE)+' Integration Figure in form '+form+'.png', bbox_inches='tight')
    plt.show()
...
def vmd_decom(series=None,alpha=2000,tau=0,K=5,DC=0,init=1,tol=1e-7,re_decom=True,re_imf=0,draw=True):
    ...
    if draw:
        ...
        fig.align_labels()
        plt.tight_layout()
        if (re_decom==False): plt.savefig(FIGURE_PATH+file_name+'VMD Result.png', bbox_inches='tight')
        else: plt.savefig(FIGURE_PATH+'IMF'+str(re_imf)+' VMD Re-decomposition Result.png', bbox_inches='tight')
        plt.show()
...
def plot_all(lstm_type,pred_ans):
    ...
    plt.legend()
    plt.savefig(FIGURE_PATH+lstm_type+' LSTM forecasting results.png', bbox_inches='tight')
    plt.show()
    return 
...
def LSTM_pred(data=None,draw=True,uni=False,show_model=True,train_set=None,next_pred=False,ahead=1):
    ...
    if draw and PERIODS != 0:
        ...
        plt.savefig(FIGURE_PATH+fig_name+' LSTM loss chart.png', bbox_inches='tight')
        plt.show()
        ...
        plt.savefig(FIGURE_PATH+fig_name+' LSTM forecasting result.png', bbox_inches='tight')
        plt.show()
...
def plot_acf_pacf(series=None):
    ...
    #Save the figure
    plt.savefig(FIGURE_PATH+'ACF and PACF of Series.png', bbox_inches='tight')
    plt.tight_layout() 
    plt.show()
